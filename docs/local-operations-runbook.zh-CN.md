...
状态：公开用户操作指南<br>
适用范围：Open Nova `v1.0.1` · macOS 本地 Runtime
...
- 已发布的 `v1.0.1` tag 和 Release 保持不可变。
- 本指南的全新安装命令同时固定 `v1.0.1` bootstrap 和 Open Nova 源码完整 commit。
- `v1.0.0` 已撤回，只保留用于审计，不应继续安装或推荐。

> [!IMPORTANT]
> 固定源码 commit 可以防止 Open Nova 源码随未来 `latest` Release 漂移，但第三方依赖仍按发布配置在安装时解析，因此不等同于整个 Python 依赖环境逐字节可复现。
...
1. bootstrap 来自不可变 `v1.0.1` tag；
2. 实际 Open Nova 源码固定为完整 commit `82bbdbd83e35724441c7005dfc0b555d413fcf93`。

> [!NOTE]
> 显式提供 commit 会跳过 bootstrap 对未来 `latest` Release 和 `WITHDRAWN` 标记的动态检查。这是固定源码内容所需的明确取舍；如项目未来撤回该版本，应以 GitHub 安全公告和 Release 状态为准。
...
- 无参数只显示计划；
- `--dry-run` 会运行 bootstrap 和安装器预演，但冷缓存时主要展示源码获取计划，不等同于完整候选版本端到端验证；
- 只有 `--apply` 会执行真实更新事务。
...
Open Nova `v1.0.1` 没有产品级一键卸载器。不要只删除 `~/.open-nova`，否则可能遗留：
