...
class ReleaseTestHarnessTests(unittest.TestCase):
    ...
    def test_release_runner_isolates_runtime_clock_secret_store_and_launchctl(self):
        ...
        self.assertIn('"--without-pip"', runner)
        self.assertIn('"ensurepip", "--upgrade"', runner)
        self.assertIn('"--python",', runner)
        self.assertIn('"pip==26.1.2"', runner)
        ...
        self.assertIn('"ACTANARA_RUN_REAL_SYSTEMD_TESTS": "0"', runner)
        ...
        self.assertIn('"TMPDIR": str(temporary_root)', runner)
        self.assertIn('patch.object(tempfile, "tempdir", str(temporary_root))', runner)
